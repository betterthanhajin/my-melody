...
class Particle {
  ...
  update(p5: p5Types, target: Attractor, deltaTime: number, G: number) {
    ...
    const force = toTarget
      .copy()
      .normalize()
      .mult((G * target.mass * Particle.mass) / distanceSquared);

    // Acceleration = Force / mass
    const acceleration = force
      .copy()
      .div(Particle.mass)
      .mult(target.forceInversion);

    // p = p0 + v0 * t + 1/2 * a * t^2
    this.position
      .add(this.velocity.copy().mult(deltaTime))
      .add(acceleration.copy().mult((deltaTime * deltaTime) / 2));
    ...
    this.color = p5.lerpColor(
      ...
      Particle.finalColor,
      this.velocity.mag() / Particle.maxColorVelocity
    );
  }
